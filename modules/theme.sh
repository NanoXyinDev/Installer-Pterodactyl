...
install_theme() {
  ...
  ui_info 'Oke, Panel aman. Sekarang gue pasang theme tanpa mengubah komponen React bawaan.'
  ...
  if ! python3 - "${layouts[@]}" <<'PYTHON'
from pathlib import Path
import re, sys

css_tag = '<link rel="stylesheet" href="/zxvcode-panel.css">'
js_tag = '<script src="/zxvcode-panel.js" defer></script>'
...
build_theme() {
  ...
  cd "$panel"
  ui_info 'Gue tidak memaksa rebuild React. Theme ini berjalan sebagai layer CSS/JS, jadi source Panel tetap aman.'
  ui_info 'Kalau dependency frontend tersedia, gue cuma cek build production sebagai langkah tambahan.'
  if [[ -f package.json ]] && command -v yarn >/dev/null 2>&1; then
    yarn build:production >/dev/null
    ui_success 'Frontend selesai dibuild ulang.'
  elif [[ -f package.json ]] && command -v npm >/dev/null 2>&1; then
    npm run build:production >/dev/null
    ui_success 'Frontend selesai dibuild ulang.'
  else
    ui_info 'Yarn/npm tidak tersedia. Tidak masalah — theme tetap bisa dipasang karena file React tidak perlu diubah.'
  fi
}

rebuild_and_apply_theme() {
  ui_section 'REBUILD & TERAPKAN THEME'
  if build_theme; then
    ui_info 'Build sudah beres. Sekarang gue pasang ulang theme dan cek Blade sekali lagi.'
    if install_theme; then
      ui_success 'Selesai. Theme aktif lagi dan Panel sudah dicek setelah pemasangan.'
    else
      ui_error 'Theme belum dipasang ulang karena pemeriksaan gagal. Panel dibiarkan pada kondisi aman.'
      return 1
    fi
  else
    ui_error 'Build frontend gagal. Gue tidak akan menyentuh theme yang sedang aktif.'
    return 1
  fi
}
...
t=t.replace('<link rel="stylesheet" href="/zxvcode-panel.css">\n','')
t=t.replace('<script src="/zxvcode-panel.js" defer></script>\n','')
