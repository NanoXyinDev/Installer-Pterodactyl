...
install_theme() {
  local panel="${PTERODACTYL_DIRECTORY:-/var/www/pterodactyl}"
  local layout="$(find_panel_layout)"
  ...
  [[ -n "$layout" ]] || {
    ui_error 'Supported Panel layout was not found.'
    ui_info 'Checked templates/wrapper.blade.php, layouts/base.blade.php and layouts/app.blade.php.'
    return 1
  }
...
brand = '''    <div class="zxv-protect-brand" aria-label="ZXV PROTECT">
    ...
    <div id="zxv-welcome-toast" class="zxv-welcome-toast" role="status" aria-live="polite" aria-atomic="true">
        ...
        <div class="zxv-welcome-copy">
            ...
            <span>SELAMAT DATANG DI PROJECT ZXV PROTECT 👋</span>
        </div>
    ...
    (() => {
        ...
        if (!toast) return;
        const key = 'zxv-protect-welcome-v1-{{ Auth::id() }}';
        const close = () => {
...
if 'zxvcode-panel.css' not in text:
    if '</head>' not in text:
        raise SystemExit('selected layout has no </head> tag')
    text = text.replace('</head>', css + '</head>', 1)

if 'zxv-protect-brand' not in text:
    pos = text.find('<body')
    if pos < 0:
        raise SystemExit('selected layout has no <body> tag')
    end = text.find('>', pos)
    if end < 0:
        raise SystemExit('selected layout has malformed <body> tag')
    text = text[:end + 1] + '\n' + brand + text[end + 1:]
...
PYTHON
  ...
  ui_success "Clean dark theme + ZXV PROTECT branding installed using $(basename "$layout")."
  ui_info 'The theme is loaded as a standalone stylesheet; a frontend rebuild is not required.'
}

theme_menu() {
  while true; do
    ui_section 'PANEL APPEARANCE'
    printf '  [01] Apply clean dark theme + ZXV PROTECT branding\n  [02] Backup current theme files\n  [03] Rebuild frontend assets\n  [00] Back\n\n'
