...
zxv_roles_install(){
  ...
  install -m 0644 "$ROLE_ROOT/migrations/2026_09_09_000002_harden_primary_admin.php" "$migration_dir/2026_09_09_000002_harden_primary_admin.php"
  install -m 0644 "$ROLE_ROOT/migrations/2026_09_09_000003_remove_root_admin_from_zxv_roles.php" "$migration_dir/2026_09_09_000003_remove_root_admin_from_zxv_roles.php"
  ...
  php -l "$migration_dir/2026_09_09_000002_harden_primary_admin.php" >/dev/null
  php -l "$migration_dir/2026_09_09_000003_remove_root_admin_from_zxv_roles.php" >/dev/null
...
Route::prefix('zxv')->group(function () {
    ...
    Route::post('/users', [ZxvAdminController::class, 'createUser'])->name('admin.zxv.users.create');
...
elif "name('admin.zxv.users.create')" not in text:
    dashboard_route = "    Route::get('/', [ZxvAdminController::class, 'index'])->name('admin.zxv.dashboard');"
    text = text.replace(dashboard_route, dashboard_route + "\n    Route::post('/users', [ZxvAdminController::class, 'createUser'])->name('admin.zxv.users.create');", 1)
...
text = text.replace("Route::middleware(['application-api', 'throttle:api.application'])", "Route::middleware(['application-api', 'throttle:api.application'])", 1)
...
injection='''{{-- ZXV ROLE MENU --}}\n@if(\\Pterodactyl\\Support\\ZxvRole::canOpenDashboard(Auth::user()))\n<li class="header">PENGELOLAAN ZXV</li>\n<li class="{{ ! starts_with(Route::currentRouteName(), 'admin.zxv') ?: 'active' }}"><a href="{{ route('admin.zxv.dashboard') }}"><i class="fa fa-dashboard"></i> <span>Dashboard ZXV</span></a></li>\n@endif\n@if(\\Pterodactyl\\Support\\ZxvRole::canCreateServer(Auth::user()))\n<li><a href="{{ route('admin.servers.new') }}"><i class="fa fa-plus-circle"></i> <span>Buat server</span></a></li>\n@endif\n'''
if marker not in s:
...
PYTHON
  ...
  (cd "$PANEL_DIR" && php artisan tinker --execute="DB::table('zxv_roles')->updateOrInsert(['user_id'=>1],['role'=>'ceo','assigned_by'=>1,'updated_at'=>now(),'created_at'=>now()]); DB::table('users')->where('id',1)->update(['root_admin'=>1]);") >/dev/null 2>&1 || { ui_error 'Primary admin ID 1 gagal di-hardening.'; return 1; }
  (cd "$PANEL_DIR" && php artisan optimize:clear >/dev/null 2>&1) || true
  ui_success 'ZXV Role System aktif: USER → ADP → OWNER → CEO.'
  ui_info 'ADP: hanya Buat server. OWNER: Dashboard + ADP management. CEO: akses penuh ke dashboard admin + kelola OWNER/ADP. Dashboard juga menampilkan startup script tiap server.'
}

zxv_roles_audit(){
  local panel="$PANEL_DIR" failed=0
  ui_section 'CEK ROLE & AKSES'
  for file in "$panel/app/Support/ZxvRole.php" "$panel/app/Http/Middleware/ZxvRoleGate.php" "$panel/app/Http/Controllers/Admin/ZxvAdminController.php" "$panel/resources/views/admin/zxv/dashboard.blade.php" "$panel/database/migrations/2026_09_09_000001_upgrade_zxv_server_scripts.php" "$panel/database/migrations/2026_09_09_000002_harden_primary_admin.php" "$panel/database/migrations/2026_09_09_000003_remove_root_admin_from_zxv_roles.php"; do
...
roles_menu(){
  while true; do
    ui_section 'ROLE & DASHBOARD ADMIN'
