...
protection_audit(){
  ...
  if [[ -f "$routes" ]] && grep -q 'ZxvPrimaryAdminOnly::class' "$routes"; then
    ui_success 'Sensitive admin route guard: OK'
  else
  ...
  if [[ -f "$routes" ]] && grep -q "'prefix' => 'nodes'.*ZxvPrimaryAdminOnly::class" "$routes"; then
    ui_success 'Direct /admin/nodes/view/{id}: protected'
  else
...
protection_menu(){
  while true; do
    ...
    case "$p" in
      ...
      3) protection_audit;;
      4) if [[ -d /var/backups/zxvcode-ptero ]]; then find /var/backups/zxvcode-ptero -type f | wc -l | xargs printf '  Backups: %s\n'; else ui_warning 'No backups yet.'; fi;;
      0|00) return;;
