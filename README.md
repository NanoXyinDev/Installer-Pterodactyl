...
## BangSano compatibility

ZXV includes an optional compatibility bundle under `integrations/bangsano/` containing Stellar, Billing, Enigma, Elysium, Nebula and Nightcore materials from the supplied archives. These older packages are kept as inspectable bundles and are not injected into the live Panel automatically because several of them patch legacy core files.

The installer can audit and stage those packages from **Theme lama & kompatibilitas**. The safe Node wizard also incorporates the useful location/node flow without importing the older `eval`-based Wings path.
...
## UI 3.3 packaging
- ZIP distribution is flat: installer files are at archive root, with no `zxv_work/` wrapper folder.
- Main installer menu option `0` shows a ZxvCode farewell screen before exiting.


### UI 3.8
- HAI anime canvas sekarang one-shot ~2.4 detik dengan efek 3D ringan dan otomatis dibersihkan.
- Nama server dikunci menjadi `server` dan creation endpoint menolak server kedua.
- Admin ID 1 tetap menjadi CEO dan memiliki akses penuh ke Admin Panel.

### UI 3.8 update
- The HAI anime greeting is short, one-shot, and GPU-friendly with a lightweight 3D presentation.
- `ZXV OFFICIAL` has a stable 3D entrance/hover treatment and is safe on mobile.
- Managed server creation is intentionally limited to exactly one server named `server`.
- Primary admin ID `1` is explicitly hardened as `root_admin` + ZXV `ceo` (full Admin Panel access).

### UI 3.9 — Natural Sano-inspired visual
- Mengubah polish visual menjadi lebih natural: background image lokal, panel hitam transparan, spacing ringkas, dan action button merah yang lebih tenang.
- `ZXV OFFICIAL` tetap dipertahankan, tetapi efek 3D/floating berlebihan dihilangkan agar terasa lebih stabil.
- Background tema dibundel lokal di `themes/panel/background.jpg` dan dipasang ke `public/zxvcode/background.jpg`, jadi tidak bergantung pada URL gambar eksternal.
- Tampilan login, dashboard, server list, form, dan notification mengikuti bahasa visual yang sama tanpa mengubah komponen React bawaan.

## ZXV × Sano Collaboration Pack

UI 4.0 bundles the complete supplied theme set from the Sano archives: Stellar, Billing, Enigma, Elysium, Nebula, and Nightcore. The installer exposes them from **ZXV × Sano Collab** and keeps legacy archives staged rather than replacing the live Panel automatically.

Nightcore CSS/background and the supplied frontend/location/node references are included for review. Legacy installer scripts are intentionally not executed by ZXV.
