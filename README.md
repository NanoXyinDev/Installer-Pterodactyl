...
- a live dashboard overview with server totals, online/offline/busy status and resource usage bars;
- a compact per-server status chip and a lightweight resource snapshot chart that follows React SPA re-renders;
- responsive dashboard, server workspace and admin styling without replacing Pterodactyl React components or requiring an external chart library.
