(() => {
    ...
    if (!shell) {
      ...
      shell.innerHTML = `
        <div class="zxv-server-list-head">
          <div><span class="zxv-overline">DAFTAR SERVER</span><strong>Server kamu</strong><small>Server yang bisa kamu akses dari akun ini.</small></div>
  ...
  const decorateAdminOverview = () => {
    ...
    brand.innerHTML = `
      <div class="zxv-admin-overview-mark"><img src="/zxvcode/mark.svg" alt="ZXV OFFICIAL"></div>
      <div class="zxv-admin-overview-brand-copy">
        <span class="zxv-overline">IDENTITAS PANEL</span>
        <strong>ZXV OFFICIAL</strong>
        <small>© ${new Date().getFullYear()} ZXV OFFICIAL · Tampilan Panel ZXV</small>
      </div>
  ...
  const drawOverview = (cards) => {
    ...
    const markup = `
      <div class="zxv-dashboard-overview-head">
        <div><span class="zxv-overline">RINGKASAN</span><strong>Server kamu</strong><small>Resource mengikuti data yang sedang tampil di sini.</small></div>
  ...
  const decorateAdmin = () => {
    ...
    if (apiLink && !document.getElementById('zxv-official-badge')) {
      ...
      if (host) {
        ...
        badge.innerHTML = '<span class="zxv-official-dot"></span><span>ZXV OFFICIAL</span><small>Tampilan Panel</small>';
    ...
    if (isCreateServer()) {
      ...
      if (main && !document.getElementById('zxv-create-banner')) {
        ...
        banner.innerHTML = '<div><span class="zxv-overline">PENGATURAN SERVER</span><strong>Atur server dari satu tempat dengan tampilan yang lebih enak dilihat.</strong><small>Pengaturan bawaan Panel tetap dipakai, jadi alurnya tidak berubah.</small></div><span class="zxv-create-chip">ZXV OFFICIAL</span>';
        main.prepend(banner);
      }
    }
  };
  ...
  const mountHaiCanvas = () => {
    ...
    const draw = (now) => {
      ...
      ctx.fillText('Halo dari ZXV OFFICIAL', 3, 22);
      ctx.restore();
  ...
  const notify = (message) => {
    ...
    box.innerHTML = `<span class="zxv-notice-dot"></span><div><strong>ZXV OFFICIAL</strong><small>${message}</small></div>`;
