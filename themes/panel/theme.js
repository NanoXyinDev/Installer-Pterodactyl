(() => {
  ...
  const mountHaiCanvas = () => {
    ...
    const draw = (now) => {
      ...
      const t=(now-start)/1000,bob=Math.sin(t*2.2)*4,swing=Math.sin(t*3)*0.12;
      const g=ctx.createRadialGradient(400,120,10,400,120,180);g.addColorStop(0,'rgba(71,190,255,.18)');g.addColorStop(1,'rgba(71,190,255,0)');ctx.fillStyle=g;ctx.beginPath();ctx.arc(400,120,180,0,Math.PI*2);ctx.fill();
      ctx.save();ctx.translate(372,168+bob);ctx.rotate(swing*.25);
      ctx.fillStyle='#151a26';ctx.beginPath();ctx.ellipse(0,92,42,14,0,0,Math.PI*2);ctx.fill();
      ctx.fillStyle='#6d2330';ctx.beginPath();ctx.arc(0,-60,30,0,Math.PI*2);ctx.fill();ctx.fillStyle='#8b2e3b';ctx.beginPath();ctx.arc(-12,-78,12,0,Math.PI*1.6);ctx.fill();
      ctx.fillStyle='#f2c7aa';ctx.beginPath();ctx.arc(0,-54,23,0,Math.PI*2);ctx.fill();ctx.fillStyle='#111722';ctx.beginPath();ctx.moveTo(-14,-63);ctx.lineTo(-2,-73);ctx.lineTo(15,-62);ctx.lineTo(14,-50);ctx.lineTo(-10,-48);ctx.closePath();ctx.fill();
      ctx.fillStyle='#ffdf5b';ctx.beginPath();ctx.arc(-26,-36,7,0,Math.PI*2);ctx.arc(26,-36,7,0,Math.PI*2);ctx.fill();
      ctx.strokeStyle='rgba(255,80,80,.8)';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(-9,-57);ctx.lineTo(8,-53);ctx.stroke();
      ctx.fillStyle='#2b5130';ctx.fillRect(-38,-26,76,85);ctx.strokeStyle='#bcd2a8';ctx.lineWidth=5;
      for(let x=-38;x<=38;x+=18){ctx.beginPath();ctx.moveTo(x,-26);ctx.lineTo(x,59);ctx.stroke();}for(let y=-8;y<=50;y+=18){ctx.beginPath();ctx.moveTo(-38,y);ctx.lineTo(38,y);ctx.stroke();}
      ctx.strokeStyle='#202531';ctx.lineWidth=18;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(-30,8);ctx.lineTo(-60,38);ctx.stroke();ctx.beginPath();ctx.moveTo(30,6);ctx.lineTo(56,36);ctx.stroke();
      ctx.strokeStyle='#f4f7fb';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(42,50);ctx.lineTo(132,8);ctx.stroke();ctx.strokeStyle='#8ea3b8';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(42,47);ctx.lineTo(132,5);ctx.stroke();
      ctx.fillStyle='#8c2432';ctx.fillRect(-10,58,20,18);ctx.fillStyle='#f2c7aa';ctx.fillRect(-36,56,16,24);ctx.fillRect(20,56,16,24);ctx.restore();
      ctx.save();ctx.font='900 44px system-ui,sans-serif';ctx.fillStyle='rgba(247,251,255,.96)';ctx.shadowColor='rgba(62,167,255,.35)';ctx.shadowBlur=24;ctx.fillText('HAI',54,88);ctx.font='800 11px system-ui,sans-serif';ctx.fillStyle='rgba(141,166,193,.9)';ctx.shadowBlur=0;ctx.fillText('ZXV OFFICIAL • ANIME GREETING',58,108);ctx.restore();
