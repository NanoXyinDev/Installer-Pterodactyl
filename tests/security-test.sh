...
grep -q '\[ + \] 1. Pasang Panel' "$ROOT/install.sh" || fail 'main setup menu was not consolidated'
...
# ZXV dashboard route/render consistency.
if [[ -f "$ROOT/protect/roles/views/dashboard.blade.php" && -f "$ROOT/modules/roles.sh" ]]; then
  grep -q "admin.zxv.users.create" "$ROOT/protect/roles/views/dashboard.blade.php" && \
  grep -q "admin.zxv.users.create" "$ROOT/modules/roles.sh" || fail "ZXV create-user route reference is missing"
fi
