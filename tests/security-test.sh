...
grep -q 'Rebuild frontend dan terapkan ulang tema' "$ROOT/modules/theme.sh" || fail 'theme rebuild action is missing'
grep -q 'Hapus tema ZXV PROTECT' "$ROOT/modules/theme.sh" || fail 'theme uninstall action is missing'
grep -q 'Cadangkan tema saat ini' "$ROOT/modules/theme.sh" || fail 'theme backup action is missing from appearance menu'
