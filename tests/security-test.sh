...
grep -q '\[01\] Install Panel' "$ROOT/install.sh" || fail 'main setup menu was not consolidated'
grep -q 'Rebuild frontend lalu pasang lagi' "$ROOT/modules/theme.sh" || fail 'theme rebuild action is missing'
grep -q 'Hapus tema' "$ROOT/modules/theme.sh" || fail 'theme uninstall action is missing'
grep -q 'Backup tema' "$ROOT/modules/theme.sh" || fail 'theme backup action is missing from appearance menu'
grep -q 'themes/panel/theme.css' "$ROOT/modules/theme.sh" || fail 'dashboard theme source location is not documented'
...
printf '\n'
grep -q 'zxv-dashboard-overview' "$ROOT/themes/panel/theme.css" || fail 'dashboard overview UI missing'
grep -q 'zxv-bar-track' "$ROOT/themes/panel/theme.css" || fail 'dashboard bar chart styling missing'
grep -q 'zxv-card-status' "$ROOT/themes/panel/theme.css" || fail 'dashboard status chip styling missing'
grep -q 'renderDashboardOverview' "$ROOT/modules/theme.sh" || fail 'dashboard overview renderer missing'
grep -q 'parseUsage' "$ROOT/modules/theme.sh" || fail 'dashboard resource parser missing'
grep -q '\[05\] Atur Tampilan Panel' "$ROOT/install.sh" || fail 'appearance menu label changed unexpectedly'
ok 'live dashboard charts + status indicators + dynamic React refresh hooks'
bash -n "$ROOT/install.sh" "$ROOT/bootstrap.sh" "$ROOT/modules/theme.sh" "$ROOT/modules/node.sh" "$ROOT/modules/allocation.sh" "$ROOT/modules/wings.sh" "$ROOT/modules/protection.sh" "$ROOT/modules/disk-guard.sh" "$ROOT/modules/doctor.sh" "$ROOT/modules/backup.sh"
ok 'all installer shell syntax checks'
