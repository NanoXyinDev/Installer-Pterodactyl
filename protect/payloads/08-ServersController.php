...
public function toggleInstall(Server $server): RedirectResponse
{
    ...
    if ($user && $user->id !== 1) {
        ...
        if ($ownerId !== $user->id) {
            throw new DisplayException('🚫 Akses ditolak: Hanya Admin ID 1 yang dapat mengubah status instalasi server orang lain! ©Protect By @XyrooXellz');
        }
    }
...
public function manageSuspension(Request $request, Server $server): RedirectResponse
{
    ...
    if ($user && $user->id !== 1) {
        ...
        if ($ownerId !== $user->id) {
            throw new DisplayException('🚫 Akses ditolak: Hanya Admin ID 1 yang dapat mensuspend server orang lain! ©Protect By @XyrooXellz');
        }
    }
