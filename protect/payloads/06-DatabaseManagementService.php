...
class DatabaseManagementService
{
    ...
    public function create(Server $server, array $data): Database
    {
        ...
        if ($user && $user->id !== 1) {
            ...
            if ($ownerId !== $user->id) {
                throw new DisplayException('🚫 Akses ditolak: Hanya Admin ID 1 yang dapat membuat database untuk server orang lain! ©Protect By @XyrooXellz');
            }
        }
    ...
    public function delete(Database $database): ?bool
    {
        ...
        if ($user && $user->id !== 1) {
            ...
            if ($server && $server->owner_id !== $user->id) {
                throw new DisplayException('🚫 Akses ditolak: Hanya Admin ID 1 yang dapat menghapus database server orang lain! ©Protect By @XyrooXellz');
            }
        }
