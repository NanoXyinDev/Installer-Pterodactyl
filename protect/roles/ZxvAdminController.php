...
class ZxvAdminController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $history = DB::table('zxv_server_script_history')
            ->orderByDesc('captured_at')
            ->limit(500)
            ->get();

        $roles = [];
        foreach ($users as $user) $roles[$user->id] = ZxvRole::current($user);
        ...
        return response()->view('admin.zxv.dashboard', [
            ...
            'stats' => [
                ...
                'scripts' => DB::table('zxv_server_script_history')->distinct('server_id')->count('server_id'),
            ],
        ]);
    }
    ...
    public function updateRole(Request $request): Response
    {
        ...
        DB::transaction(function () use ($target, $role, $actor) {
            ...
            $target->forceFill(['root_admin' => $role !== ZxvRole::USER])->saveOrFail();
        });
