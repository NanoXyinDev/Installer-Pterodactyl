...
@section('title', 'Dashboard ZXV')
@section('content-header')
    <h1>Dashboard ZXV <small>{{ \Pterodactyl\Support\ZxvRole::label($role) }}</small></h1>
@endsection
@section('content')
<div class="zxv-admin-wrap">
    <div class="zxv-zxv-hero">
        <div class="zxv-zxv-hero-copy">
            <span class="zxv-overline">ZXV OFFICIAL · PUSAT KONTROL</span>
            <h2>Panel control yang stabil, cepat, dan jelas.</h2>
            <p>Semua aksi memakai progress feedback. Data kosong tidak lagi bikin layout patah.</p>
        </div>
        <div class="zxv-zxv-hero-progress" aria-label="Dashboard readiness">
            <span><b>SEMUA SIAP</b><i>100%</i></span>
            <div><em style="width:100%"></em></div>
        </div>
    </div>
    ...
    <div class="box zxv-admin-box">
        <div class="box-header with-border"><h3 class="box-title">Tambah pengguna</h3><span class="zxv-box-note">Role ZXV diatur dari sini</span></div>
        <div class="box-body">
            <form method="post" action="{{ route('admin.zxv.users.create') }}" class="row">
                ...
                <div class="col-md-12" style="margin-top:10px"><button class="btn btn-primary"><i class="fa fa-user-plus"></i> Buat user</button> <small class="text-muted">User baru tidak otomatis menjadi Admin Pterodactyl.</small></div>
            </form>
        </div>
    </div>
    ...
    <div class="box zxv-admin-box">
        ...
        <div class="box-body table-responsive">
            <table class="table table-hover zxv-table">
                <thead><tr><th>Pengguna</th><th>Role</th><th>Server</th><th>Akses</th><th>Perbarui</th></tr></thead>
                <tbody>
                @forelse($users as $user)
                    ...
                    <tr>
                        ...
                        <td>{{ $userRole === 'user' ? 'Pengguna Panel' : ($userRole === 'adp' ? 'Tambah Server' : ($userRole === 'owner' ? 'Dashboard + ADP' : 'Akses penuh')) }}</td>
                        <td>
                ...
                @empty
                    <tr><td colspan="5" class="zxv-empty-state">Belum ada user yang bisa dikelola.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="box zxv-admin-box">
        <div class="box-header with-border"><h3 class="box-title">Semua script server</h3><span class="zxv-box-note">Script startup masing-masing server</span></div>
        <div class="box-body table-responsive">
            <table class="table table-hover zxv-table">
                <thead><tr><th>Server</th><th>Owner</th><th>Egg</th><th>Script startup</th><th>Action</th></tr></thead>
                <tbody>
                ...
                @empty
                    <tr><td colspan="5" class="zxv-empty-state">Belum ada server.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="box zxv-admin-box">
        <div class="box-header with-border"><h3 class="box-title">Riwayat script server</h3><span class="zxv-box-note">Riwayat perubahan script per server</span></div>
        <div class="box-body table-responsive">
            <table class="table table-hover zxv-table">
                ...
                @empty
                    <tr><td colspan="5" class="zxv-empty-state">Belum ada snapshot startup script.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
