...
## UI 3.8 — BangSano compatibility merge
- Bundled supplied Stellar, Billing, Enigma, Elysium, Nebula and Nightcore compatibility materials under `integrations/bangsano/`.
- Added compatibility audit/staging menu without auto-overwriting the live Panel.
- Added safe automatic location + node wizard without the legacy `eval` command path.
- Preserved ZXV role, disk guard, anti-flood, theme backup, single-server policy, and ID 1 CEO hardening.


## UI 3.9 — Natural Visual
- Sano-inspired visual language based on the user-supplied reference ZIP.
- Local photographic background, black translucent surfaces, compact rounded controls, and restrained red actions.
- Reduced futuristic gradients and continuous motion while preserving stable ZXV branding and mobile behavior.
- Background asset is bundled locally instead of loading the reference image from an external URL.

### UI 4.0 — ZXV × Sano Collaboration Pack
- Full supplied Sano theme bundle: Stellar, Billing, Enigma, Elysium, Nebula, Nightcore.
- Dedicated collab menu with audit, per-theme staging, full bundle staging, and source references.
- Legacy executable installer paths remain excluded from automatic execution.
